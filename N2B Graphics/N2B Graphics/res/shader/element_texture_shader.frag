...
//const int DIFFUSE_COUNT = 8;
//uniform sampler2D texture_diffuse[DIFFUSE_COUNT];
//uniform int diffuse_number;

//const int SPECULAR_COUNT = 8;
//uniform sampler2D texture_specular[SPECULAR_COUNT];
//uniform int specular_number;
...
vec3 calc_directional_light(Directional_Light light, vec3 vertex_dir, vec3 vertex_pos, vec3 view_dir);
...
void main()
{
	...
	{
		light_strength += calc_point_light(point_lights[i], vertex_direction, vertex_position, view_direction);
	}
	light_strength += calc_directional_light(dir_light, vertex_direction, vertex_position, view_direction);
...
vec3 calc_directional_light(Directional_Light light, vec3 vertex_dir, vec3 vertex_pos, vec3 view_dir)
{
	return max(dot(vertex_dir, -light.direction), 0.0f) * light.strength * vec3(texture(material.texture, vertex_texture_coo)) * light.color;
}
